...
GamePauseState::GamePauseState(GameDataRef data)
{
    ...
    initBtn("Resume", sf::Vector2f(525 * SCL(this), 175 * SCL(this)), 0, [](GUIAbstract *btn, int tag){
        if (tag == 0)
            btn->getData()->machine.removeState();});
    initBtn("Home", sf::Vector2f(525 * SCL(this), 268 * SCL(this)), 1, [](GUIAbstract *btn, int tag){
        if (tag == 0)
...
void GamePauseState::initBtn(std::string text, sf::Vector2f pos, int tag, inputReactFct input)
{
    ...
    btn->setShapeTexturedRectUpdate(m_data->assets.getTexture("button"), {0, 0, 165, 52}, {0, 52, 165, 52}, {0, 52, 165, 52});
    btn->setCharacterSize(30  * SCL(this));
    btn->setTextColors(sf::Color::Black);
